...
# hardwired to change g1 and the soil moisture stress function in the control, #
...
NCPUs=4
models="Control"
base_project="calib_g1" # input in based project to modify
...
#######################################
# Input files for the various g1
# Globals:
#   lots
# Arguments:
#   None
...
for i in $(echo ${sites} |sed "s/,/ /g"); do
    ...
    # create the new actual sites input files with updated g1
...
for i in $(echo ${sites} |sed "s/,/ /g"); do
    ...
    python ${cal} ${i} ${base_project} -p "g1"
