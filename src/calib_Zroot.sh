...
# hardwired to change the rooting depth based on the Control runs, but this    #
...
NCPUs=4
models="Control"
base_project="calib_rooting_depth" # input in based project to modify
...
# now optimise the rooting depth
if [[ ! -z ${ssl} ]]; then 
...
#######################################
# Input files for the various Zbottom
# Globals:
#   lots
# Arguments:
#   None
...
for i in $(echo ${sites} |sed "s/,/ /g"); do
    ...
    # create the new actual sites input files with updated Zbottom
    wf=${data_dir}${sl}input${sl}projects${sl}${project}${sl}${i}*".csv"
    filenames=$(find ${wf})
...
#######################################
# Sample the effective rooting depth &
